...
| SHELL-02 | Inspect the canvas zone and all shell surfaces.   | The canvas uses a deterministic square grid. It has no rounded corners, CSS gradients, scanline overlay, or animated visual effect.                                                                      |
...
| ID      | Action                                                                                                                                                                                   | Expected outcome                                                                                                                                                                 |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GATE-01 | Run the Fabric scene spike in a real browser. Import or create a raster surface, add a vector shape, group them, set group opacity, transform the group, serialize, restore, and export. | The restored scene has the same object structure and group opacity. The rendered result matches the pre-serialization result at 100% zoom.                                       |
| GATE-02 | Run the raster bridge spike in a real browser. Change one raster backing surface and invalidate only its Fabric renderer.                                                                | The visible raster updates. The Fabric object identity and unrelated scene objects remain unchanged.                                                                             |
| GATE-03 | Render the checkerboard warp at 100% and 400% zoom. Inspect all eight mesh triangles and the outer bounds.                                                                               | No visible one-pixel seams appear. Mesh coverage is complete. A canceled warp restores the exact source pixels.                                                                  |
| GATE-04 | Run both local Object Selection candidates against five representative photos. Record runtime and subject-isolation result for each photo.                                               | At least one candidate isolates the intended foreground subject in at least four of five photos within two seconds on the reference computer.                                    |
| GATE-05 | Run the tile-history spike with a 256 x 256 dirty region.                                                                                                                                | Undo and redo restore identical pixel hashes. A pointer gesture can be represented by one transaction.                                                                           |
| GATE-06 | Run the JPEG target-size spike with a 4096 x 4096 composite or equivalent reference workload.                                                                                            | The quality search reaches a target within the accepted tolerance when possible and does not freeze the browser UI.                                                              |
